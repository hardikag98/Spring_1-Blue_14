...
setwd("C:/Users/zacha/OneDrive/Desktop/Classes/Spring/Financial Analytics/Homework1_FA")
...
accepted <- accepted %>% 
  mutate(CARDS_bin = ifelse(CARDS_bin == "Cheque card" |
                              CARDS_bin == "Mastercard/Euroc" |
                              CARDS_bin == "no credit cards",
                            CARDS_bin, #If one of three above: remain
                            "Other Credit Card")) #OW: group into other cat
...
# Binning of Continuous Variables - IV >= 0.02 #
...
cat_names <- names(train)[sapply(train, is.factor)] # Gathering the names of categorical variables in data #
...
train$EC_CARD_WOE <- ifelse(train$EC_CARD_bin == "0", -0.1932, 0.5599)
test$EC_CARD_WOE <- ifelse(test$EC_CARD_bin == "0", -0.1932, 0.5599)
...
rejected_scored$pred <- predict(initial_score, newdata=rejected_scored, type='response')
...
accepted$weight_ar <- ifelse(accepted$GB == 1, weight_ab, weight_ag)
rejected$weight_ar <- ifelse(rejected$GB == 1, weight_rb, weight_rg)
...
train_comb$EC_CARD_WOE <- ifelse(train_comb$EC_CARD_bin == "0", -0.2569, 0.9108)
test_comb$EC_CARD_WOE <- ifelse(test_comb$EC_CARD_bin == "0", -0.2569, 0.9108)
...
cutpoints <- quantile(accepted_scored_comb$Score, probs = seq(0,1,0.1))
accepted_scored_comb$Score.QBin <- cut(accepted_scored_comb$Score, breaks=cutpoints, include.lowest=TRUE)
Default.QBin.pop <- round(table(accepted_scored_comb$Score.QBin, accepted_scored_comb$GB)[,2]/(table(accepted_scored_comb$Score.QBin, accepted_scored_comb$GB)[,2] + table(accepted_scored_comb$Score.QBin, accepted_scored_comb$GB)[,1]*4.75)*100,2)
...
for(i in min(floor(train_comb$Score)):max(floor(train_comb$Score))){
  ...
  def[i - min(floor(train_comb$Score)) + 1] <- 100*sum(train_comb$GB[which(train_comb$Score >= i)])/(length(train_comb$GB[which(train_comb$Score >= i & train_comb$GB == 1)]) + 4.75*length(train_comb$GB[which(train_comb$Score >= i & train_comb$GB == 0)]))
  acc[i - min(floor(train_comb$Score)) + 1] <- 100*(length(train_comb$GB[which(train_comb$Score >= i & train_comb$GB == 1)]) + 4.75*length(train_comb$GB[which(train_comb$Score >= i & train_comb$GB == 0)]))/(length(train_comb$GB[which(train_comb$GB == 1)]) + 4.75*length(train_comb$GB[which(train_comb$GB == 0)]))
  prof[i - min(floor(train_comb$Score)) + 1] <- length(train_comb$GB[which(train_comb$Score >= i & train_comb$GB == 1)])*(-cost) + 4.75*length(train_comb$GB[which(train_comb$Score >= i & train_comb$GB == 0)])*profit
}
